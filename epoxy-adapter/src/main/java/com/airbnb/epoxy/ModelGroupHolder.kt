...
class ModelGroupHolder : EpoxyHolder() {
    ...
    private fun areSameViewType(model1: EpoxyModel<*>, model2: EpoxyModel<*>?): Boolean {
        return VIEW_TYPE_MANAGER.getViewType(model1) == VIEW_TYPE_MANAGER.getViewType(model2)
    }

    private fun getViewHolder(parent: ViewGroup, model: EpoxyModel<*>): EpoxyViewHolder {
        val viewType = VIEW_TYPE_MANAGER.getViewType(model)
    ...
    fun unbindGroup() {
        ...
        viewHolders.forEachIndexed { index, _ -> removeAndRecycleView(index) }
    ...
    companion object {
        ...
        private val VIEW_TYPE_MANAGER = ViewTypeManager()
